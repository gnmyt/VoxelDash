...
/**
 * Resource management (mods + datapacks). All Minecraft/Forge specifics — the
 * loaded-mod list, jar metadata parsing (mods.toml vs mcmod.info), and the
 * mods/config/datapacks folders — come from {@link ForgeCompat}; the file
 * handling here is version-independent. Datapacks are only offered when the
 * running line supports them ({@link ForgeCompat#supportsDatapacks()}).
 */
public class ResourcePipeImpl implements ResourcePipe {

    private static final Gson GSON = new Gson();
    private static final Type MAP_TYPE = new TypeToken<Map<String, Object>>() {}.getType();
    private static final Set<String> BUILTIN_MODS = new HashSet<>(Arrays.asList("minecraft", "forge", "mcp", "fml", "voxeldash"));
    ...
    private List<Resource> getMods() {
        ...
        Map<String, ForgeCompat.ModEntry> loadedByFile = new HashMap<>();
        Map<String, ForgeCompat.ModEntry> loadedById = new HashMap<>();
        for (ForgeCompat.ModEntry info : compat().loadedMods()) {
        ...
        if (jarFiles != null) {
            for (File jarFile : jarFiles) {
                ...
                ForgeCompat.ModEntry loaded = loadedByFile.get(jarFile.getName());
                ForgeCompat.ModEntry meta = loaded == null ? compat().parseJarMeta(jarFile) : null;
        ...
        if (disabledFiles != null) {
            for (File disabledFile : disabledFiles) {
                ...
                ForgeCompat.ModEntry meta = compat().parseJarMeta(disabledFile);
                resources.add(new Resource.Builder()
    ...
    private List<ConfigFile> getModConfigFiles(String fileName) {
        ...
        ForgeCompat.ModEntry meta = compat().parseJarMeta(jarFile);
