...
public class VersionCompat implements FabricCompat {
    ...
    public void runCommand(String command) {
        runOnMainThread(() -> {
            ...
            s.getCommandManager().execute(s.getCommandSource(), clean);
        });
    }
    ...
    public ArrayList<OnlinePlayer> onlinePlayers() {
        ...
        for (ServerPlayerEntity player : s.getPlayerManager().getPlayerList()) {
            ...
            long playtime = player.getStatHandler().getStat(Stats.CUSTOM.getOrCreateStat(Stats.PLAY_ONE_MINUTE)) * 50L;
            players.add(new OnlinePlayer(
                    player.getName().getString(),
                    player.getUuid(),
                    worldId(player.getServerWorld()),
                    ip,
                    player.getHealth(),
    ...
    public void kickPlayer(String playerName, String reason) {
        runOnMainThread(() -> {
            ...
            if (player != null) player.networkHandler.disconnect(new LiteralText(reason));
        });
    }
    ...
    public void setGamemode(String playerName, String gamemode) {
        runOnMainThread(() -> {
            ...
            if (player != null) {
                try {
                    GameMode mode = GameMode.byName(gamemode.toLowerCase(), null);
    ...
    public void setOp(String playerName) {
        runOnMainThread(() -> {
            ...
            if (p != null) s.getPlayerManager().addToOperators(p);
        });
    }
    ...
    public void deOp(String playerName) {
        runOnMainThread(() -> {
            ...
            if (s == null) return;
            GameProfile p = s.getUserCache().findByName(playerName);
            if (p != null) s.getPlayerManager().removeFromOperators(p);
        });
    }
            ...
            for (String name : banList.getNames()) {
                ...
                if (entry != null) {
                    players.add(new BannedPlayer(name, profile.getId(), entry.getReason(),
                            null, entry.getExpiryDate(), entry.getSource()));
                }
            }
        } catch (Exception ignored) {
        }
    ...
    public void banPlayer(String playerName, String reason) {
        runOnMainThread(() -> {
            ...
            ServerPlayerEntity player = s.getPlayerManager().getPlayer(playerName);
            if (player != null) player.networkHandler.disconnect(new LiteralText("You have been banned: " + reason));
        });
    }
    ...
    private static String worldId(ServerWorld world) {
        ...
        return id != null ? id.toString() : ("minecraft:dim_" + type.getRawId());
    }
    ...
    private ServerWorld findWorld(String worldId) {
        ...
        for (ServerWorld world : s.getWorlds()) {
            ...
            if (id.equals(worldId) || id.endsWith(":" + worldId) || path.equals(worldId)) {
                return world;
            }
        }
        return null;
    ...
    private World convertWorld(ServerWorld world) {
        ...
        if (type == DimensionType.THE_NETHER) environment = "NETHER";
        else if (type == DimensionType.THE_END) environment = "THE_END";
    ...
    public WorldStats worldStats() {
        ...
        int worldCount = 0, chunks = 0;
        for (ServerWorld world : s.getWorlds()) {
            worldCount++;
            chunks += world.getChunkManager().getLoadedChunkCount();
        }
        return new WorldStats(worldCount, 0, chunks);
    }
    ...
    public File datapacksFolder() {
        MinecraftServer s = server();
        if (s != null) return new File(new File(s.getRunDirectory(), s.getLevelName()), "datapacks");
